...
test("owner submits a fixed nonblocking systemd service restart", async t => {
  ...
  await createRestart().commands.restart.handle(f.inv, f.ctx);
...
test("non-owner cannot restart the service", async t => {
  const f = fixture(); t.after(f.restore);
  await createRestart().commands.restart.handle({...f.inv, message: {...f.inv.message, senderId: "999"}}, f.ctx);
...
test("cancellation after the notice prevents spawning", async t => {
  ...
  await assert.rejects(() => Promise.resolve(createRestart().commands.restart.handle(f.inv, f.ctx)));
  assert.equal(f.calls.length, 0);
});
