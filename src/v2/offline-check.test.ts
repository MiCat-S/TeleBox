...
test("offline CLI integrates built-ins, SQLite reload, services and admission without user data", t => {
  ...
  const result = spawnSync(process.execPath, [path.resolve(__dirname, "../../dist/v2/index.js"), "--check"], {
  ...
  assert.deepEqual(output.loaded, ["alias", "check", "help", "loglevel", "memory", "prefix"]);
...
test("CLI rejects unknown modes and does not touch the current directory", t => {
  ...
  for (const args of [[], ["--start"], ["--check", "--start"]]) {
    const result = spawnSync(process.execPath, [path.resolve(__dirname, "../../dist/v2/index.js"), ...args], {cwd: root, encoding: "utf8", timeout: 5000});
