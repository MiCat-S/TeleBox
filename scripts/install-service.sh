...
if [[ "${1:-}" == "--help" && $# == 1 ]]; then
  printf '%s\n' 'Usage: bash scripts/install-service.sh' \
    ...
    '/root/telebox and sibling /root/TeleBox-Plugins.' \
    'Installs and starts telebox-v2. Refuses active or enabled services.'
  exit 0
fi
[[ $# == 0 ]] || { echo "Unsupported arguments" >&2; exit 2; }
[[ $(uname -s) == Linux && $EUID == 0 ]] || { echo "Run on Linux as root" >&2; exit 1; }
root=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd -P)
[[ "$root" == /root/telebox ]] || { echo "Install the core repository at /root/telebox" >&2; exit 1; }
cd "$root"
...
[[ -d /root/TeleBox-Plugins && -d node_modules ]] || { echo "Install dependencies and the sibling plugin repository first" >&2; exit 1; }
...
exec 9>/run/lock/mi-box-install.lock
/usr/bin/flock -n 9 || { echo "Another installer is running" >&2; exit 1; }
state=$(/usr/bin/systemctl show telebox-v2 -p ActiveState --value)
case "$state" in inactive|failed|"") ;; *) echo "Mi Box service is $state; do not install over a running instance" >&2; exit 1 ;; esac
enabled=$(/usr/bin/systemctl is-enabled telebox-v2 2>/dev/null || true)
...
unit=/etc/systemd/system/telebox-v2.service
[[ ! -L "$unit" ]] || { echo "Refusing a symlink service unit" >&2; exit 1; }
/usr/bin/systemd-analyze verify deploy/systemd/telebox-v2.service
backup=$(mktemp -d /root/mi-box-install.XXXXXX)
printf 'Backup: %s\n' "$backup"
...
restore() {
  ...
  if [[ $result != 0 && "$changed" == true ]]; then
    ...
    /usr/bin/systemctl disable --now telebox-v2 || true
...
trap restore EXIT
changed=true
printf '%s\n' 'Building Mi Box and plugins...'
/usr/bin/node scripts/package-v2-daily.cjs > "$backup/build.log" 2>&1
/usr/bin/node dist/v2/index.js --check > "$backup/check.log" 2>&1
install -m 644 deploy/systemd/telebox-v2.service "$unit"
/usr/bin/systemctl daemon-reload
/usr/bin/systemctl reset-failed telebox-v2 || true
/usr/bin/systemctl enable --now telebox-v2
invocation=$(/usr/bin/systemctl show telebox-v2 -p InvocationID --value)
...
for ((attempt=0; attempt<30; attempt++)); do
  ...
  /usr/bin/systemctl is-active --quiet telebox-v2 || break
  sleep 2
done
[[ "$ready" == true ]] || { echo "Startup not ready; inspect $backup/startup.log" >&2; exit 1; }
sleep 5
/usr/bin/systemctl is-active --quiet telebox-v2
[[ $(/usr/bin/systemctl show telebox-v2 -p InvocationID --value) == "$invocation" ]]
changed=false
printf '%s\n' 'Mi Box is ready and enabled at boot.' \
  'Verify .help and .ping in Telegram.' \
  'Logs: journalctl -u telebox-v2 -f'
